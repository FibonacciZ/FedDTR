...
class BuildThree(nn.Module):
    def __init__(self, base, head, args):
        super(BuildThree, self).__init__()

        self.base = base
        self.head = head
        self.g_base = copy.deepcopy(self.base)
        self.g_head = copy.deepcopy(self.head)

    def forward(self, x):
        out = self.base(x)
        p_out = self.head(out)
        g_out = self.g_head(out)
