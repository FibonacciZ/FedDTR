...
from flcore.servers.serverghp import FedGhp
from flcore.servers.servertext import FedText
from flcore.servers.serveriter import FedIter
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif args.algorithm == "FedGhp":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BuildThree(args.model, args.head, args).to(args.device)
            server = FedGhp(args, i)

        elif args.algorithm == "FedText":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BuildThree(args.model, args.head, args).to(args.device)
            server = FedText(args, i)

        elif args.algorithm == "FedIter":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BuildThree(args.model, args.head, args).to(args.device)
            server = FedIter(args, i)
